<template>
  ...
  <div class="media-source" 
    ...
    <div class="media-source-details">
      ...
      <div class="banner-container pull-left" v-if="mediaSource.banner !== 'default'">
        <div :style="'background-image: url(' + cdnUrl + '/posters/' + mediaSource.banner + '.png' + ')'"></div>
      </div>

      <div v-else class="small-default-preview-banner pull-left">
    ...
    <div class="ms-actions">
      <button
        v-if="!selected"
        @click.prevent.stop="$parent.showRemoveMediaSourceModal($event, mediaSource)"
        type="button"
      ...
      <div class="selected-btn pull-right" v-if="selected">
        <i class="icofont-check pull-right"></i>
        Selected
      </div>
...
.media-sources-list > div.media-source {
    margin-top: 10px;
}
